...
- `\"`: double quote when strings are enclosed in double quotes (`"Nay, I can\"t either"`)
...
#2010
  ...
  library(plyr)
  counts10 <- count(bag10)
  counts10$len <- nchar(as.character(counts10$x))
...
#2016
  ...
  counts16 <- count(bag16)
  counts16$len <- nchar(as.character(counts16$x))
...
```{r}
  counts10 <- counts10[!(counts10$x %in% stoplist),]
  counts16 <- counts16[!(counts16$x %in% stoplist),]
  
  head(counts10,10)
  head(counts16,10)
```
At this point, we've arrived at words that are more commonly presidential sounding.