...
To assess accuracy, we will install and load the `plotROC` library, which calculates accuracy and uses `ggplot2` to graph the ROC. In order to calculate the ROC, we will create a new data frame `input` that is comprised of the labels for the test set `ytest` and the predicted probabilities `test.prob`. 
```{r}
#Load libraries
  library(ggplot2)
  library(plotROC)
...
#Set up test data frame
  input <- data.frame(ytest = ytest, test.prob)
```
...
#Base object
  base <- ggplot(input, aes(d = ytest, m = test.prob))

#ROC
  roc <- base + geom_roc() + style_roc()
...
#Assemble a well-formatted table
  library(xtable)
  tab <- data.frame(model = unique(input$model), train = calc_auc(roc.train)$AUC, test = calc_auc(roc.test)$AUC)
