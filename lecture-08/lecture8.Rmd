...
The inverse of the logistic function is the *log odds ratio* or *logit link*, which is written as:

$$logit(F(z)) = log(\frac{F(z)}{1-F(z)}) = w_0 + w_1 x_1 + w_2 x_2   $$
where *log* is a natural logarithm transformation. This relationship is particularly important as it allows for conversion of probabilities into odds and vice versa. Furthermore, the underlying weights of the logistic regression can be interpretted using *Odds Ratios* or *OR*. 

To start, we define *odds* as the exponentiated $F(x)$:

$$odds = e^{F(x)} = e^{w_0 + w_1 x_1 + w_2 x_2}$$

For a marginal 1 unit increase in $x_1$, the OR is expressed as the $odds(x+1)$ over $odd(x+0)$:
